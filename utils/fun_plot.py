...
def plot_price_km_db(marca,modelo):
    ...
    Mantiene el diseÃƒÆ’Ã‚Â±o/estilo original.
    """
    import math
    ...
    # === Ordenar por km para lÃƒÆ’Ã‚Â­nea de ajuste (sin cambios) ===
    ...
    if len(kms_arr) >= 3:
        try:
            ...
            eq_text = f'y = {popt[0]:.0f}AÂ·e^({popt[1]:.7f}AÂ·x) + {popt[2]:.0f}'
        except Exception as e:
            eq_text = f'Ajuste no disponible: {e}'
    else:
        eq_text = 'Datos insuficientes para ajuste'

    # === ClasificaciÃƒÆ’Ã‚Â³n chollo/caro (igual) ===
    ...
    # === ConstrucciÃƒÆ’Ã‚Â³n de fuentes (igual) ===
...
# --------------------------------------------------------------------------------------------------------------------------------------------------------
# Redefiniciones: las siguientes funciones consumen datos pre-calculados por fun_model.modelfit
...
def plot_price_km_by_year_json(model_json, marca, modelo):
    ...
    kms, precios, urls, years, prov_names, tags, residuals, zscores = [], [], [], [], [], [], [], []
    for it in registros:
        precio = it.get('price', None)
        km_val = it.get('km', None)
        url = it.get('url', None)
        year_val = _coerce_year(it.get('year', None))
        if precio is None or km_val is None:
            continue
        kms.append(int(km_val))
        precios.append(float(precio))
        urls.append(url if isinstance(url, str) and url.strip() else 'javascript:void(0)')
        ...
        tags.append(it.get('tag', 'normal'))
        residuals.append(float(it.get('residual', 0.0)))
        zscores.append(float(it.get('z', 0.0)))
    ...
    tags_arr = np.array(tags, dtype=object)[order]
    resid_arr = np.array(residuals, dtype=float)[order]

    fit_x = fit_y = None
    eq_text = ''
    if len(registros) > 0:
        try:
            fx = registros[0].get('fit_x', None)
            fy = registros[0].get('fit_y', None)
            fit_x = np.array(fx) if fx is not None else None
            fit_y = np.array(fy) if fy is not None else None
            eq_text = registros[0].get('eq_text', '')
        except Exception:
            pass
    ...
    data_with_year = dict(
        ...
        km_fmt=[_fmt_km(v) for v in kms_arr[mask_year]],
        year_fmt=[str(int(v)) for v in years_arr[mask_year]],
        residual=resid_arr[mask_year],
        tag=[t for (t,m) in zip(tags_arr, mask_year) if m],
        border_color=border_colors[mask_year],
    ...
    if fit_x is not None and fit_y is not None:
        try:
            mad = float(np.median(np.abs(resid_arr - np.median(resid_arr))))
            sigma = (1.4826*mad) if mad > 0 else float(np.std(resid_arr))
            low = fit_y - 1.96*sigma
            high = fit_y + 1.96*sigma
            band_src = ColumnDataSource(dict(x=fit_x, low=low, high=high))
            band = Band(base='x', lower='low', upper='high', source=band_src, level='underlay', fill_color='#BBBBBB', fill_alpha=0.18, line_color=None)
...
def plot_price_km_by_province_json(model_json, marca, modelo):
    ...
    kms, precios, urls, years, prov_names, tags, residuals = [], [], [], [], [], [], []
    def _coerce_year(y):
        try:
            y = int(str(y).strip())
            if 1980 <= y <= 2035:
    ...
    # Agrupar provincias con 3+ items, resto 'Otros'
    counts = {}
    for p in prov_names_arr:
        counts[p] = counts.get(p, 0) + 1
    prov_group_arr = np.array([p if counts.get(p, 0) >= 3 else 'Otros' for p in prov_names_arr], dtype=object)
    factors = sorted(list(set(prov_group_arr.tolist())))
    ...
    # color por provincia (Pastel)
    pastel = list(Pastel2[7]) if hasattr(Pastel2, '__getitem__') else list(Pastel2)
    # ajustar tamaño de paleta a factores
    ...
    if len(data_with_year['km']) > 0:
        ...
        # Chollo y Timo en la leyenda (excluye 'Otros')
        ...
        # Provincias ordenadas por numero de registros (desc) y sin '0' ni 'Otros'
        count_map = {p: int(np.sum(prov_group_arr == p)) for p in set(prov_group_arr.tolist())}
